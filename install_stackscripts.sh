...
# $4 = TDL
# $5 node name
# $6 healthcheck git repo
# $7 main.yml git repo
#
# the above variables are provided in the stackscript code
...
heathcheck () {
    ...
    mkdir -p /opt/scripts
    cd /opt/scripts
    git -c http.sslVerify=false clone "$health" health
    ...
    crontab -l > /tmp/mycron
    echo '*/1 * * * * run-one /bin/bash /opt/scripts/health/healthcheck.sh $5 $4 > /dev/null 2>&1' >> /tmp/mycron
    crontab /tmp/mycron
    rm /tmp/mycron
